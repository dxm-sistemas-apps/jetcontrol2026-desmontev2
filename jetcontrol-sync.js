...
(function () {
  ...
  var TEMPO_LIMITE_MS = 8000;  // cobre a busca de TODAS as páginas de dados; se não responder nesse tempo, segue só com dados locais
  ...
  // -----------------------------------------------------------------------
  // CORREÇÃO DE CORRIDA (LOCAL PENDENTE x SINCRONIZAÇÃO):
  // O envio para a nuvem é adiado por ATRASO_ENVIO_MS (agrupa digitação
  // rápida). Antes, o carimbo de tempo local (meta[chave]) só era gravado
  // quando o envio de fato acontecia — ou seja, durante essa janela de
  // espera, o dado já estava salvo no localStorage, mas o "meta" ainda
  // achava que a última atualização era mais antiga. Se o ciclo de
  // verificação automática (a cada poucos segundos) rodasse nesse meio-
  // tempo, ele podia comparar contra esse carimbo desatualizado e
  // sobrescrever a edição local recém-feita com um valor mais antigo vindo
  // da nuvem, antes mesmo dela ter sido enviada.
  //
  // Agora o carimbo é gravado assim que a edição é agendada (aqui embaixo),
  // não quando o envio de fato ocorre. Assim, qualquer sincronização que
  // rode durante a espera já "sabe" que o dado local é mais recente e não
  // o sobrescreve.
  // -----------------------------------------------------------------------
  function agendarEnvio(chave, valor, apagar) {
    ...
    var carimboDestaEdicao = Date.now();
    var meta = lerMeta();
    meta[chave] = carimboDestaEdicao;
    salvarMeta(meta);

    timersPendentes[chave] = setTimeout(function () {
      enviarAgora(chave, valor, apagar, carimboDestaEdicao);
    }, ATRASO_ENVIO_MS);
  }

  // "carimbo" é o horário definido no momento em que a edição foi agendada
  // (não quando o envio de fato acontece), para ficar consistente com o
  // valor já gravado em meta[chave] por agendarEnvio.
  function enviarAgora(chave, valor, apagar, carimbo) {
    var agora = carimbo || Date.now();
    var mascara = apagar
  ...
  // -----------------------------------------------------------------------
  // CORREÇÃO DE PAGINAÇÃO: o Firestore devolve no máximo "pageSize"
  // documentos por chamada (aqui, 300) e indica em "nextPageToken" se há
  // mais páginas. Antes, apenas a primeira página era lida — se a coleção
  // "dados" passasse de 300 registros, os excedentes ficavam invisíveis
  // para a sincronização (não eram baixados, nem contados para decidir o
  // que já existe na nuvem). Esta função busca todas as páginas antes de
  // continuar.
  // -----------------------------------------------------------------------
  function buscarTodosOsDocumentos(pageToken, acumulado) {
    acumulado = acumulado || [];
    var url = URL_BASE + '?pageSize=300' + (pageToken ? '&pageToken=' + encodeURIComponent(pageToken) : '');

    return fetch(url).then(function (resposta) {
      if (!resposta.ok) throw new Error('Firestore respondeu ' + resposta.status);
      return resposta.json();
    }).then(function (dados) {
      var documentosDaPagina = (dados && dados.documents) || [];
      var todos = acumulado.concat(documentosDaPagina);
      if (dados && dados.nextPageToken) {
        return buscarTodosOsDocumentos(dados.nextPageToken, todos);
      }
      return todos;
    });
  }
  ...
  function sincronizarUmaVez() {
    ...
    // O limite de tempo agora cobre a busca de TODAS as páginas (não só a
    // primeira), já que uma coleção grande pode precisar de mais de uma
    // chamada para ser lida por completo.
    var buscaComLimiteDeTempo = Promise.race([
      buscarTodosOsDocumentos(),
      new Promise(function (_, rejeitar) {
        setTimeout(function () { rejeitar(new Error('tempo de espera esgotado')); }, TEMPO_LIMITE_MS);
      })
    ]);

    return buscaComLimiteDeTempo.then(function (documentos) {
      var meta = lerMeta();
